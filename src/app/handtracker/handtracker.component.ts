  ...
  */
  ...
  startDetection(){
    this.startVideo().then(()=>{
        //The default size set in the library is 200px. Change here or use styling
  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            if (openhands > 1) this.detectedGesture = "Removed Answer";
            else if(openhands == 1) this.detectedGesture = "Paper";
            
            if (closedhands > 1) this.detectedGesture = "";
            else if(closedhands == 1) this.detectedGesture = "Rock";
            
            if (pointing > 1) this.detectedGesture = "Guns";
            else if(pointing == 1) this.detectedGesture = "Scissors";
            
            if (pinching > 1) this.detectedGesture = "";
            else if(pinching == 1) this.detectedGesture = "Lizard";

            if (openhands == 1 && closedhands == 1) this.detectedGesture = "Wizard";

            if (pointing == 1 && closedhands == 1) this.detectedGesture = "Ogre";
